...
function ActivityPresentation({ row }: { row: Row<ReportPreview> }) {
    ...
    return (
        <div className="flex flex-col gap-6">
            {reports.map(report => {
                return <div key={report.ID} className="flex items-start gap-2">
                    <UserAvatar fallback={report.ae_code} image={report.image} sales_unit={report.sales_unit} className="size-6 mt-0.5" fallbackClassName="text-[0.5rem]" />
                    <div className="w-full flex flex-col gap-1">
                        <p className="max-w-2xl whitespace-break-spaces leading-tight">{report.activity}</p>
