...
const EditCompany = () => {
  ...
  const availableUnitHeads: List[] = useMemo(() => {
    if (!users || !salesUnits) return [];
    console.log(salesUnits);
    const results = users.filter((user) => {
      return (
        ![1, 2, 5].includes(user.role.role_id) &&
        !units.some(
          (unit) =>
            Number(unit.unit_head.id) === user.ID ||
            unit.unit_members.some((member) => Number(member.id) === user.ID)
        ) &&
        !salesUnits.some(
          (unit) =>
            Number(unit.sales_unit_head?.user_id) === user.ID ||
            unit.sales_unit_members?.some(
  ...
  }, [users, salesUnits, units]);

  const availableUnitMembers: List[] = useMemo(() => {
    if (!users || !salesUnits) return [];
    const results = users.filter((user) => {
      return (
        ![1, 2].includes(user.role.role_id) &&
        !units.some(
          (unit) =>
            unit.unit_head.id === String(user.ID) ||
            unit.unit_members.some((member) => member.id === String(user.ID))
        ) &&
        !salesUnits.some(
          (unit) =>
            (unit.sales_unit_head?.user_id ?? "") === String(user.ID) ||
            unit.sales_unit_members?.some(
              (member) => (member.user_id ?? "") === String(user.ID)
            )
        )
      );
    });
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    updateCompany(
      { id: companyID, newCompany: updatedCompany },
      {
        onSuccess: () => {
          toast({
  ...
  return (
    <Page className="flex flex-col gap-4">
      <Helmet>
        <title>Edit {code} | Sales Platform</title>
      </Helmet>
