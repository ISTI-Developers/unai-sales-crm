...
const AddCompany = () => {
  const { companies, insertCompany } = useCompany();
  ...
  const setMembers = (id: string, index: number, action?: string) => {
    ...
    setSalesUnits((prev) => {
      ...
      const updatedMembers = action
        ...
        : [
            ...unit.unit_members,
            availableSalesUnits.find((item) => item.id === id),
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const response = await insertCompany(newCompany);
  ...
  return (
    <Page className="flex flex-col gap-4">
      <Helmet>
        <title>Add Company | Sales CRM</title>
      </Helmet>
        ...
        <div className="flex items-center gap-2">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (companyNames.includes(value.replace(".", "").toLowerCase())) {
                setError(
