...
export const useGeneratePowerpoint = () => {
    ...
    const print = async () => {
        ...
        try {
            ...
            const addOns = new Map<string, AddOn>(selectedOptions.add_ons.map(addOn => [addOn.key, addOn]))
            const bookingTerms = selectedOptions.settings.booking_terms.map(term => term.duration)
            const currency = selectedOptions.currency_exchange.currency ?? "PHP";
            const equivalent = selectedOptions.currency_exchange.equivalent ?? 1;
            ...
            for (const [index, site] of selectedSites.entries()) {
                ...
                const rate = printingCost[regions[prefix] as keyof typeof printingCost]

                const dims = site.size
                    .match(/\d+(\.\d+)?/g)
                    ?.map(Number)

                const printCost = dims?.reduce((acc, n) => acc * n, rate) ?? 0

                const updatedCost = applyExchangeRate(printCost, equivalent);
                ...
                addText(slide, formatNumber(site.traffic_count ?? 0), {
                    w: Inches(6.6),
                    h: labelHeight,
                    x: Inches(21.47),
                    y: Inches(7.06),
                ...
                addText(slide, formatNumber(site.vicinity_population ?? 0), {
                    w: Inches(6.6),
                    h: labelHeight,
                    x: details2ndColumnSection,
                    y: Inches(7.06),
                ...
                if (selectedOptions.settings.rate_basis === "MULTIPLE") {
                    ...
                    const rateRows = bookingTerms.map((term) => {
                        ...
                        const monthlyRate = applyPriceAdjustment(baseRate, { amount: adjustment.value, type: adjustment.type })
                        const displayRates: PptxGenJS.TableRow = [];
                        if (materialRate) {
                            const value = materialRate.type === "FREE" ? materialRate.value > 0 ? `${materialRate.value}x free` : formatAmount(updatedCost, {
                                style: "currency",
                        ...
                        if (installationRate) {
                            displayRates.push({
                                text: installationRate.value > 0 ? `${installationRate.value}x free` : '---',
                                options: {
                                    align: "center" as PptxGenJS.HAlign,
                                    fontSize: 10,
                                    fill: { color: "FFFFFF" },
                    ...
                    const [text] = bookingTerms.map(term => {
                        const materialRate = materialPrinting?.rates[term];
                        const installationRate = installationDismantling?.rates[term];
                    ...
                    if (materialPrinting?.rates[bookingTerms[0]].value === 0) {
                ...
                const map = queryClient.getQueryData<string>([
                    "map",
                    site.site_code,
                ]);
                if (map) {
                    ...
                    slide.addImage({
                        data: map,
                        x: Inches(21.68),
                        y: selectedOptions.settings.rate_basis === "MULTIPLE" ? Inches(12.84) : Inches(11.02),
                        w: selectedOptions.settings.rate_basis === "MULTIPLE" ? Inches(6.97) : Inches(8),
                        h: selectedOptions.settings.rate_basis === "MULTIPLE" ? Inches(6.97) : Inches(8),
                    })
                    if (site.ideal_view) {
                        slide.addText("View Google Map", {
                            ...
                            h: Inches(0.73),
                            x: Inches(21.68),
                            y: selectedOptions.settings.rate_basis === "MULTIPLE" ? Inches(19.18) : Inches(18.29),
                            align: "center",
                            color: "25589D",
                            fontSize: 11,
                            isTextBox: true,
    ...
    return { applyOptions, print, applyExchangeRate };
}